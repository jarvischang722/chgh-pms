...
$(function () {
    ...
    $('#day_remodal_select_1').datepicker({
        ...
        onSelect: function onSelect(fd, date) {
            ...
            content = fd;
            if (!_.isEmpty(fd)) {
                $(".date_checklist_pool").append('<div class="checkbox_list"><label>' +
                    '<input type="checkbox" value="' + fd + '" class="option-input checkbox select_all_check_check_2 select_batch_date_checkbox" /> ' +
                    content + ' </label></div>');
            }
        }
    });
