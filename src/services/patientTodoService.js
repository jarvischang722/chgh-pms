 ...
 * */
 ...
 exports.getPatientTodo = function(patient_person_id, callback){

    //有傳要指定哪個病患的話

        DBAgent.query("QRY_TODO_BY_PATIENT_ID",
 ...
 * 更新病人待辦事項資料
 * */
exports.updatePatientTodo = function(patientInfos, update_user,callback){
                    ...
                    if(validResult === ""){
                        ...
                        async.parallel(
                            [function(callback){
                                ...
                                DBAgent.query("QRY_TODO_BY_PATIENT_ID",{patient_person_id:patientInfoObject["patient_person_id"]} , function(err , rows){
                            ...
                            , function (err, result) {
                                ...
                                if(err){
                                    ...
                                    DBAgent.query("UPD_PATIENT_todo_information_BY_PATIENT_ID",
                                        patientInfoObject,
                                        function(err , rows){
 ...
 * 插入病人待辦事項資料
 * */
exports.insertPatientTodo = function(patientTodoInfos, update_user,callback){

    var insertFuncsArray=Array();

    if(!patientTodoInfos instanceof Array){
        //check data type
        callback("資料格式錯誤，非JsonArray");
        ...
        _.each(patientTodoInfos, function(patientInfo){
            ...
            if(!_.isUndefined(patientInfo)) {
                ...
                insertFuncsArray.push(function (callback) {
                    ...
                    //驗證及得到驗證結果字串從當前的病患資訊中
                    var validResult=insertValidAndGetMsgFromPatientTodoInfoObject(patientInfo);
                        ...
                        async.parallel(
                            ...
                            , function (err, result) {
                                ...
                                if(err){
                                    //插入病房失敗
                                    Logger.error(err);
                                    resultMsg += "插入身份證字號:"+patientInfoObject["patient_person_id"]+"的病人待辦事項資料時，發生不知名錯誤，請嘗試重新插入!\n"
                                    ...
                                    DBAgent.query("INS_todo_information",
                                        patientInfoObject,
                                        function(err , rows){
                                            ...
                                            if(err){
                                                Logger.error(err);
                                                console.log(err);
                                                resultMsg += "插入身份證字號:"+patientInfoObject["patient_person_id"]+"的病人待辦事項資料時，發生不知名錯誤，" +
                                                    "請確認身份證字號已存在於系統後，再重新插入!\n"

                                            }else{

                                                resultMsg += "插入身份證字號:"+patientInfoObject["patient_person_id"]+"的病人待辦事項資料成功!\n"
                                            }
                                    ...
                                    //非0代表該資料已存在，無法新增
                                    resultMsg += "插入身份證字號:"+patientInfoObject["patient_person_id"]+"的病人待辦事項資料時失敗，該身份證已經存在!\n"
 ...
 */
function insertValidAndGetMsgFromPatientTodoInfoObject(patientInfo){

    var reusltMsg="";

    //必填資訊
    var patient_person_id=validator.isPersonID(patientInfo["patient_person_id"]);
    var todo_content=validator.isString(patientInfo["todo_content"]);
    ...
    isStatus:function (input){
        ...
        if(input===""){
            ...
            for(var i=0;i<toArray.length;i++){
                ...
                if( toArray[i] !=="檢查異常" && toArray[i]  !=="待辦事項" && toArray[i]  !=="跌倒" && toArray[i]  !=="隔離"){
...
function getInsertPatientInfoObject(patientInfo){
    ...
    var todo_content=patientInfo["todo_content"];
...
function getUpdatePatientInfoObject(patientInfo){
    ...
    var patient_person_id=patientInfo["patient_person_id"];
    var todo_content=patientInfo["todo_content"] || null;
